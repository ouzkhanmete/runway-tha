...
- **Multi-app and multi-worker by design.** Onboarding an app is a single `POST /apps`; registration **resolves the app's name and existence** in one iTunes Lookup API call (the reviews feed carries neither), returning `400` if the id doesn't exist — so the name is set immediately and the worker only ingests reviews. The worker self-discovers the app on its next tick; workers claim apps atomically, so you can run several without double-processing, and the scheduler never overlaps its own ticks.
- **Pre-populated demo.** The full Docker stack runs a one-shot **seed** step before the app starts, registering the current US top-free app ids (through the same `register` path) — so a reviewer lands on a UI that already has apps, names, and reviews filling in.
...
- **Names + existence are resolved at registration.** `POST /apps` calls the iTunes Lookup API once: `resultCount` confirms the app exists (a numeric-but-nonexistent id is rejected with `400`) and `trackName` is written to `apps.name` on insert — so the name is present immediately and the worker no longer fills it. The reviews feed can't do this job: it returns 200 with an empty feed for a missing id (indistinguishable from an app with no recent reviews) and carries no name. Manual adds, the form, and the top-apps **seed** all go through this same `register` path. ([`docs/api.md`](docs/api.md#post-apps) · [`docs/decisions.md`](docs/decisions.md))
...
- Startup order: `postgres` → `migrate` (applies migrations) → **`seed`** (registers the current US top-free app ids, names and all) → `worker` + `api` → `web`.
- So the UI is **pre-populated**: open it and you'll see ~10 apps (with names) whose reviews fill in over the first few worker ticks. You can also add any App Store ID via the form or `POST /apps`.
