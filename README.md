...
- **Multi-app and multi-worker by design.** Onboarding an app is a single `POST /apps` (id only); the worker self-discovers it on the next tick and **resolves the app's display name** from the iTunes Lookup API (the reviews feed doesn't carry it). Workers claim apps atomically, so you can run several of them without double-processing, and the scheduler never overlaps its own ticks.
- **Pre-populated demo.** The full Docker stack runs a one-shot **seed** step before the app starts, inserting the current US top-free app ids — so a reviewer lands on a UI that already has apps, names, and reviews filling in.
...
- **Apps are id-first; the worker resolves the name.** Every app is registered with just an id (manual add, the form, or the top-apps **seed**). Because the reviews feed has no app name, the worker backfills `apps.name` from the iTunes Lookup API once per app, in the same tick it ingests reviews — so manual and seeded apps behave identically. The UI shows the id first, then the name a moment later. ([`docs/etl.md`](docs/etl.md#app-name-enrichment) · [`docs/decisions.md`](docs/decisions.md))
...
- Startup order: `postgres` → `migrate` (applies migrations) → **`seed`** (inserts the current US top-free app ids) → `worker` + `api` → `web`.
- So the UI is **pre-populated**: open it and you'll see ~10 apps whose names and reviews fill in over the first few worker ticks. You can also add any App Store ID via the form or `POST /apps`.
