...
import { AppStoreFeedClient } from "../../src/infrastructure/feed/app-store-feed-client";
import { rssPage } from "../helpers/fixtures";
...
function makeFetch(pages: Array<{ status: number; body?: object }>) {
  let call = 0;
...
describe("AppStoreFeedClient", () => {
  test("builds the correct URL per page", async () => {
    ...
    const client = new AppStoreFeedClient({
      fetch: fakeFetch,
      baseUrl: BASE_URL,
      maxPages: 2,
      maxRetries: 1,
  ...
  test("aggregates pages 1..maxPages and returns correct pagesFetched", async () => {
    const p1 = rssPage({ entries: [{ id: "1", rating: 5 }, { id: "2", rating: 4 }] });
